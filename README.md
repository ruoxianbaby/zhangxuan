## 张轩
我的在线简历

## 个人信息  
 - 张轩/男/1993  
 - 专科/镇江高等专科大学  
 - 工作年限：5年  
 - 手机：15061690110  
 - Email：ruo123xian@gmail.com  
 - 微信号：m18361815801  
 - LeetCode: [ruoxianbaby](https://leetcode-cn.com/ruoxianbaby/)  
 - Github：[ruoxianbaby](http://github.com/ruoxianbaby)  
 - 期望职位：PHP中高级程序员/技术经理/JavaScript前端工程师  
 - 期望薪资：16k~20k  

## 技能清单  
以下均为我熟练使用的技能  

- Web开发：PHP/Python/JavaScript  
- Web框架：Yii2/ThinkPHP/Laravel/Swoft  
- 前端框架：Vue/Element-ui/Ant-design/Lodash  
- 前端工具：Webpack/Vue-cli  
- 运维技能: Docker/Shell  
- 数据库相关：MySQL/Redis  
- 版本管理、文档和自动化部署工具：Git/Git-Workflow/Markdown  

## 其他技能  
- 熟悉设计模式，算法，微服务，分布式锁，消息队列  
- 注重开发规范，开发流程和技术选型的合理性  
- 熟悉Mysql锁机制，性能优化，索引原理，全文搜索，数据库拆分  
- 学完《JavaScript高级程序设计》《数据结构与算法的JavaScript实现》《深入理解ES6》《JavaScript的函数式编程》 
- 阅读《高性能MySQL》《深入PHP、面向对象、设计模式与实践》《Yii2官方文档》《Modern PHP》《Laravel文档》 
- 逛GitHub,看优秀源码,常关注知乎掘金编程话题  
 
## 工作经历

### 上海云频科技（2021年1月） 

#### 此课项目  

##### 智能对练  
- 调用众慧树接口，对接用户和后台管理API
- 前端调用后台的token，获得权限去打开智能对练的小程序  

##### 视频回放  
- 调用agora接口，云端生成视频
- 调用阿里云oss，存放视频，并生成回看文件  

##### [闯关学习](http://testck.yunpintech.com)  
- 很有意思的功能，能增加用户的参与度
- 主要难点在于数据结构的复杂度

### 上海超越 (2020年2月-2020年5月)  

#### 立顿茶小程序
- 对接微信公众号后台接口  
- 腾讯视频领取视频会员接口  
- 京东小程序接口
- 管理后台

#### 捷豹路虎小程序
- 维护更新功能,发布新版本
- 日常新活动更新,小程序与后台代码的更新

### 上海犀亿科技（2018年10月-2019年12月）

#### 犀金项目
> 本人负责技术选型, 项目的架构, 开发规范的制定, 开发人员配备, 代码仓库管理  

- 使用php7.2, Yii2, Composer管理包, Restful风格接口, rds-mysql的集群, rds-redis等技术
- 使用环信sdk, 创蓝短信sdk, Ali-pay/Wechat-pay的支付sdk
- 使用阿里云Oss-sdk
 - [官网](xykj1.com)
 - [接口文档](https://ruoxianbaby.github.io/xiyi-documents/%E7%8A%80%E9%87%91/)
 
#### 随手花项目
> 本人负责技术选型, 项目的架构, 开发规范的制定, 开发人员配备, 代码仓库管理 

- 使用PHP7.2, Yii2, Composer管理包, Restful接口风格, RDS-Mysql的集群, RDS-Redis等技术
- 使用阿里云oss-sdk
 - [官网](sshua.com)
 - [接口文档](https://ruoxianbaby.github.io/xiyi-documents/%E9%9A%8F%E6%89%8B%E8%8A%B1/)  

### 上海言通 （ 2017年10月 ~ 2018年8月 ）

#### 呼叫中心

- 老网站
维护服务器的稳定,解决紧急bug,做出新功能。亮点,使用git帮助公司稳定生产.

- 新网站
独立设计了新网站的技术架构,带着八个组员开发新呼叫系统.  
基于yii2框架做了前后分离,前端使用Vue大大缩短了开发周期.  
本人解决了几大难题,前后分离的跨域,代码复用,继承的多态问题,完善公共模块,model的复用问题
- [技术选型](https://github.com/ruoxianbaby/web-design/blob/master/web-architecture.md)  
- [架构设计](https://github.com/ruoxianbaby/web-design/blob/master/yt-web-developer-document.md)  
- [项目预览](http://ai167.ytcall.net/backend.php?r=login/index)

## 致谢
感谢您花时间阅读我的简历，期待能有机会和您共事。
